...
function setupEventListeners() {
    ...
    // Modal buttons - using event delegation
    document.addEventListener('click', function(e) {
        // Masjid modal
        if (e.target.matches('#add-masjid-btn')) {
            openModal('masjid');
        }
        // Jamaat modal
        else if (e.target.matches('#add-jamaat-btn, #add-new-jamaat-btn')) {
            openModal('jamaat');
        }
        // A'maal modal
        else if (e.target.matches('#add-amaal-btn')) {
            openModal('amaal');
        }
        // User modal
        else if (e.target.matches('#add-user-btn')) {
            openModal('user');
        }
        // Modal close buttons
        else if (e.target.matches('.modal-close, .modal .btn-secondary')) {
            const modal = e.target.closest('.modal');
            if (modal) modal.style.display = 'none';
        }
        // Modal save buttons
        else if (e.target.matches('#save-masjid')) {
            saveMasjid();
        }
        else if (e.target.matches('#save-jamaat')) {
            saveJamaat();
        }
        else if (e.target.matches('#save-amaal')) {
            saveAmaal();
        }
        else if (e.target.matches('#save-user')) {
            saveUser();
        }
    });
...
// Remove the duplicate event listeners at the bottom of your script
// (the ones that were causing the "Cannot read properties of null" errors)
...
// DOM Elements
        ...
        const modals = {
            masjid: {
                openBtn: document.getElementById('add-masjid-btn'),
                modal: document.getElementById('masjid-modal'),
                closeBtn: document.getElementById('cancel-masjid'),
                saveBtn: document.getElementById('save-masjid')
            },
            jamaat: {
                openBtn: document.getElementById('add-jamaat-btn'),
                modal: document.getElementById('jamaat-modal'),
                closeBtn: document.getElementById('cancel-jamaat'),
                saveBtn: document.getElementById('save-jamaat')
            },
            amaal: {
                openBtn: document.getElementById('add-amaal-btn'),
                modal: document.getElementById('amaal-modal'),
                closeBtn: document.getElementById('cancel-amaal'),
                saveBtn: document.getElementById('save-amaal')
            },
            user: {
                openBtn: document.getElementById('add-user-btn'),
                modal: document.getElementById('user-modal'),
                closeBtn: document.getElementById('cancel-user'),
                saveBtn: document.getElementById('save-user')
            }
        };
        ...
        function initCharts() {
            ...
            // Masjid Chart
            const masjidCtx = document.getElementById('masjidChart').getContext('2d');
            const masjidChart = new Chart(masjidCtx, {
                type: 'bar',
                data: {
                    labels: ['Masjid Al-Huda', 'Masjid Al-Noor', 'Masjid Al-Falah', 'Masjid Al-Sunnah', 'Masjid Al-Rahman'],
                    datasets: [{
                        label: 'Jamaats',
                        data: [12, 8, 6, 4, 3],
